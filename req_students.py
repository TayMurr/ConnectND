...
if __name__ == "__main__":
	# Get all NetIDs (could contain professor and staff NetID)
	#with open("/afs/nd.edu/common/etc/passwd") as f:
	#	NetIDs = [ str(index) + '-' + student.rstrip().split(':')[0] for index, student in enumerate(f) if student.rstrip().split(':')[0] != None ] ] 
		# Change into a geneartor
		#NetIDs = ( student.rstrip().split(':')[0] for student in f )
	...
	output = open("ND_directory.json", 'w')
